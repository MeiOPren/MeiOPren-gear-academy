...
mod tests {
    ...
    fn first_player_is_program() {
        ...
        system.init_logger();
        let program = Program::current(&system);
        system.mint_to(PROGRAM_OWNER, 450000000000000000);
        let _ = program.send(PROGRAM_OWNER, PEBBLES_INIT.clone());
        let random_count = 1;
        program.send(PROGRAM_OWNER, PebblesAction::Turn(random_count));
        system.run_next_block();
        let game_state: GameState = program.read_state(b"").expect("Failed to read state");
        assert_eq!(14, game_state.pebbles_count - random_count);
    }
    ...
    fn give_up() {
        ...
        let program = Program::current(&system);
        system.mint_to(PROGRAM_OWNER, 450000000000000000);
        program.send(PROGRAM_OWNER, PEBBLES_INIT.clone());
        system.run_next_block();
